...
class BanAppealModal(ui.Modal):
    ...
    async def callback(self, inter: Interaction):
        ...
        emb = Embed(title=f"{inter.user.name} ({inter.user.id})", description=f"```\n{self.appeal.value}\n```", color=self.bot.colors["light_orange"])
        if self.media.value:
        ...
        await self.bot.db.appeal.set_submission_time(inter.user.id, now.timestamp())

        self.bot.loop.call_later(60 * 60 * 24 * 7 * 2, asyncio.create_task, generic_decide(self.bot, inter.user.id, appeal_message.id))

        await inter.followup.send(f"Your appeal has been sent. You will receive a response in 2 weeks", ephemeral=True)
...
class BanAppeal(commands.Cog):
    ...
    async def create_views(self):
        ...
        pending_decisions = await self.bot.db.appeal.get_all_pending_decisions()
