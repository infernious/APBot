...
class ModerationCommands(commands.Cog):
    ...
    async def warn(
        self, interaction: discord.Interaction, member: discord.Member, *, reason: str, attachment: discord.Attachment = None
    ):
        """
        ...
        warn_log.add_field(
            ...
            value=f"{interaction.user.name}#{interaction.user.discriminator} ({interaction.user.mention})",
            inline=False,
        )
    ...
    async def unmute(
        ...
        try:
            ...
            if member_config["infractions"][-1]["type"] == "mute":
                ...
                member_config["infractions"][-1][
                    "unmute moderator"
                ] = f"{interaction.user.mention} ({interaction.user.name}#{interaction.user.discriminator})"
                if attachment:
    ...
    async def forceban(
        self, interaction: discord.Interaction, user_id: str, *, reason: str, attachment: discord.Attachment = None
    ):
        """
        ...
        ban_log.add_field(
            name=f"Responsible Moderator: ",
            value=f"{interaction.user.name}#{interaction.user.discriminator}" f" ({interaction.user.mention})",
            inline=False,
        )
        ban_log.timestamp = datetime.datetime.now()
    ...
    async def warnings(self, interaction: discord.Interaction, user: discord.User):
        ...
        await interaction.followup.send(
            f"Complete, all infractions shown! {user.mention} has " f"{member_config['infraction_points']} infraction points."
        )
    ...
    async def editip(self, interaction: discord.Interaction, member: discord.Member, change: int):
        ...
        await interaction.response.send_message(
            f"Adding {change} to {member.mention}'s infraction points, "
